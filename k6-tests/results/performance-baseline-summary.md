...
### Bullet Points (주니어/신입)

```
하루하나 - 일일 학습 서비스 (Spring Boot, MySQL)

• k6 + Prometheus + Grafana 기반 성능 테스트 환경 구축 (Load/Stress Test)
• 대용량 데이터(500만 건) 환경에서 API 성능 측정 및 병목 지점 분석
• Stress Test(500 VU)에서 HikariCP 커넥션 풀 포화(Pending 290건) 원인 분석
  → 300 VU 병목 시작점 식별 및 커넥션 풀·인덱스 최적화 방향 도출
• Load Test(250 VU) 결과 p95 27ms, 에러율 0% 달성

기술: k6, Prometheus, Grafana, Spring Boot, MySQL, HikariCP, Docker
```

### 면접 한줄 설명

> "EC2 2vCPU, 500만 건 데이터 환경에서 k6 성능 테스트로 HikariCP 커넥션 풀 포화 병목을 식별하고, Grafana 모니터링을 통해 Pending 290건·CPU 99.9% 연쇄 반응 구조를 분석했습니다."
